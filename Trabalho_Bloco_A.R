...
linhaDolar <- ggplot(data=df_imp_dolar_brent_brasil, aes(x=mes)) +
  geom_line(aes(y = CotacaoDolar, group=1), color = "darkred", size=2)+
  ggtitle("Cotação dólar 2018-07 até 2020-07") +
  ylab("Cotação")

linhaBrent <- ggplot(data=df_imp_dolar_brent_brasil, aes(x=mes)) +
...
linhaVarProdutor <- ggplot(data=df_imp_dolar_brent_brasil, aes(x=mes)) +
